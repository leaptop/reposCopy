...
void found(std::vector<record*> Array) {
	...
	for (i = 0; true;) {
		...
		for (j = 0; j < 20; ++j) {
			if (i < Array.size()) {
				printf("%5d. %32s %18s %6d %11d            %10s", i + 1, Array[i]->fio,
					Array[i]->street, Array[i]->house_number,
					Array[i]->apartment_number, Array[i]->date);
				++i;
			}
...
void data_to_sort(char *DATE, unsigned short int HOUSE_NUMBER, unsigned short int APARTMENT_NUMBER, char *DESTINATION) {
	*DESTINATION++ = DATE[6];
	*DESTINATION++ = DATE[7];
	*DESTINATION++ = DATE[3];
	*DESTINATION++ = DATE[4];
	...
	*DESTINATION++ = DATE[1];
	...
	for (unsigned short int i = 4; i > 0; --i) {
		*DESTINATION++ = HOUSE_NUMBER / int(pow(10, i)) % 10 + '0';
	}
	*DESTINATION++ = HOUSE_NUMBER % 10 + '0';

	for (unsigned short int i = 4; i > 0; --i) {
		*DESTINATION++ = APARTMENT_NUMBER / int(pow(10, i)) % 10 + '0';
	}
	*DESTINATION++ = APARTMENT_NUMBER % 10 + '0';
}
...
bool weight_compare(const weight_element FIRST, const weight_element SECOND) {
	if (SECOND.data->front()->house_number > FIRST.data->front()->house_number) {
		return 1;
	}
	else {
		if (SECOND.data->front()->house_number == FIRST.data->front()->house_number
			&& SECOND.data->front()->apartment_number > FIRST.data->front()->apartment_number) {
			return 1;
		}
	}
...
void dictionary_building(std::vector<weight_element> &DICTIONARY, std::vector<record*> ARRAY) {
	...
	for (i = 0; i < ARRAY.size(); ++i) {
		...
		while (it != DICTIONARY.end()) {
			if (it->data->front()->house_number == ARRAY[i]->house_number
				&& it->data->front()->apartment_number == ARRAY[i]->apartment_number) {
	...
	std::stable_sort(DICTIONARY.begin(), DICTIONARY.end(), weight_compare);

	for (int i = 0; i < DICTIONARY.size(); ++i) {
		printf("%d: %d %d\n", DICTIONARY[i].weight, DICTIONARY[i].data->front()->house_number, DICTIONARY[i].data->front()->apartment_number);
	}
...
void add(tree_vertex **POINTER, weight_element *A) {
	while (*POINTER != nullptr) {
		if ((*POINTER)->data->front()->house_number > A->data->front()->house_number) {
			POINTER = &((*POINTER)->left);
		}
		else {
			if (A->data->front()->house_number > (*POINTER)->data->front()->house_number) {
				POINTER = &((*POINTER)->right);
			}
			else {
				if ((*POINTER)->data->front()->apartment_number > A->data->front()->apartment_number) {
					POINTER = &((*POINTER)->left);
				}
				else {
					if (A->data->front()->apartment_number > (*POINTER)->data->front()->apartment_number) {
						POINTER = &((*POINTER)->right);
					}
					else {
						break;
...
void A2(tree_vertex *&ROOT, std::vector<weight_element> ARRAY, int L, int R) {
	int w = 0;
	int s = 0;
	int i;
...
std::vector<record*> tree_search(tree_vertex *VERTEX_POINTER, unsigned short KEY1, unsigned short KEY2) {
	if (VERTEX_POINTER == nullptr) {
		...
		if (VERTEX_POINTER->data->front()->house_number > KEY1) {
			return tree_search(VERTEX_POINTER->left, KEY1, KEY2);
		}
		else {
			if (KEY1 > VERTEX_POINTER->data->front()->house_number) {
				return tree_search(VERTEX_POINTER->right, KEY1, KEY2);
			}
			else {
				if (VERTEX_POINTER->data->front()->apartment_number > KEY2) {
					return tree_search(VERTEX_POINTER->left, KEY1, KEY2);
				}
				else {
					if (KEY2 > VERTEX_POINTER->data->front()->apartment_number) {
						return tree_search(VERTEX_POINTER->right, KEY1, KEY2);
					}
					else {
	...
	while (true) {
		...
		if (menu_key == '3') {
			...
			while (true) {
				...
				if (menu_key == '2') {
					unsigned int tree_search_key1, tree_search_key2;

					system("CLS");

					dictionary_building(dictionary, founded_list);

					A2(search_tree, dictionary, 0, dictionary.size() - 1);

					printf("Enter house_number apartment_number\n");

					std::cin >> tree_search_key1;
					std::cin >> tree_search_key2;

					founded_list = tree_search(search_tree, tree_search_key1, tree_search_key2);
...
int main() {
	...
	for (i = 0; i < 4000; ++i) {
		...
		p->index = i;
		data_to_sort(Array[i].date, Array[i].house_number, Array[i].apartment_number, p->data);
	}
