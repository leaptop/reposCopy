// C2.10.cpp : Defines the entry point for the console application.
//
...
main()
{
	...
	while (!feof(text))                                  // пока не конец файла
	{
		...
		m = getc(text);                      // чтение символа из файла     
		if ((m == 225) || (m == 226) || (m == 227) || (m == 228) || (m == 230) || (m == 231) || (m == 234) || (m == 235) || (m == 236) || (m == 237) || (m == 239) || (m == 240) || (m == 241) || (m == 242) || (m == 244) || (m == 245) || (m == 246) || (m == 247) || (m == 248) || (m == 249) || (m == 250) || (m == 252) || (m == 233))
		{
			for (i = 0; i<k; i++)
			{
				...
				{
					putc(m, sogl);   // запись в файл glass символа m
					pop[j];
					j--;
					m = getc(text);
					e++;
		...
		else if ((m == 224) || (m == 229) || (m == 232) || (m == 238) || (m == 243) || (m == 253) || (m == 254) || (m == 255))
		{
			for (i = 0; i<k; i++)
			{
				...
				{
					putc(m, glas);   // запись в файл glass символа m
					m = getc(text);
					t++;
				}
			}
	...
	cout << endl << "перевёртыш" << endl;
	fclose(nov);
	rewind(nov);
	nov = fopen("перевертыш.txt", "r");
	for (i = 0; i<e + t + 4; i++)
	{
		m = getc(nov);
		putchar(m);
