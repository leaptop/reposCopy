...
float func(float x) { return (mas[0]*x*x+mas[1]*x+mas[2]); }
/* Производная от уравнения второго порядка */
float d_func(float x) { return (2*mas[0]*x+mas[1]); }
...
void main()
{
   ...
   else if(mas[0]==0 && mas[1]!=0 && mas[2]!=0)
   ...
   if(C<A || C>B) printf("There is no solution.");
   else printf("\nSolution %.3f", C);
   getch();
}